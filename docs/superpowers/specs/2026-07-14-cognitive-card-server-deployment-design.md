...
3. 生成包含应用 wheel、14 个精确 runtime wheels、依赖锁、单一受治理 `ops/wheel_audit.py` 和必要部署文件的 release 归档及 SHA-256 摘要；不兼容于旧格式的 wheelhouse/target 字段使用 `cognitive-card-server-release-v2` schema，manifest 逐文件绑定 wheelhouse 和审计器，并记录 implementation、Python version、ABI、platforms 与 only-binary 目标元数据。构建器与安装器共用该审计器：检查 ZIP 路径/唯一性/类型/大小上限、精确 `.dist-info`、METADATA 的 Name/Version、WHEEL 的声明标签、RECORD 全覆盖的 SHA-256/大小，并将解压标签限定为 CPython 3.12/Linux x86_64，允许 pure `any`、`manylinux2014_x86_64`、不高于 `manylinux_2_28_x86_64` 的显式标签和合法 `abi3` 下限。归档排除 sdist、额外/重复/版本漂移的 distribution、Git 元数据、缓存、测试临时文件、本地数据库和凭据。
