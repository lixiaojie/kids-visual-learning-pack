# Kids Visual Learning Pack — Spec v2

> 基于 2026-05-12 实际代码状态生成，与 v1 的差异：Phase 1 重构已完成、校验体系已落地、导航 UX 已修复、资产路径已对齐。本文档可直接交给其他模型续写下阶段计划。
...
| 部署 | Vercel（自动）+ 自有服务器（rsync），相对路径 `base: "./"` |
...
│   │   │   ├── data/
│   │   │   │   ├── loaders/           # locale-merge.ts, load-map.ts, load-topic.ts
│   │   │   │   ├── schema/            # topic.schema.json, i18n-overlay.schema.json
│   │   │   │   ├── topics/            # 6 个 topic JSON（render-ready）
│   │   │   │   ├── locales/en-US/     # 6 个 overlay JSON（锁字段已清理）
│   │   │   │   ├── exploration-map.json
│   │   │   │   ├── topic-registry.json  # 18 个 topic（6 ready + 12 planned）
...
│   ├── check-image-assets.mjs         # PNG/WebP 配对（本地开发用）
│   ├── build-vercel.sh                # Vercel 构建
│   └── deploy.sh                      # 服务器部署（rsync）
└── docs/
    ├── spec-v2.md                     # 本文件
    ├── architecture-review-v1.md      # 架构审查报告
...
**7 个世界**（1 动画入口 + 6 知识世界），**18 个 topic**（6 ready + 12 planned）：
...
| 身体世界 | #EF4444 红 | blood-cells-3d | immune-system, heart, digestion |
...
| 人造系统世界 | #06B6D4 青 | llm-kids-basics | internet-message, robots, search-engine-kids |
...
└── TopicPage
    ...
    ├── RepresentativeObjects (41 行, activeObjectId 受控)
...
└── load-topic.ts     — getTopic(slug, locale): 6 个 topic JSON 静态 import + merge
...
- **Manifest**: `image-generation-manifest.json`（43 个 asset，全部 WebP 到位）
- **降级链**: GeneratedImage 组件 → WebP → PNG → null
- **命名**: `{world}-{topic}-{description}-v02.webp`
- **生图词包**: V2.1（43 张全量，通用前缀/后缀 + 芋头兄弟识别系统）
- **Topic assets 字段**: 已全部更新为 v02 WebP 路径
...
- 世界卡片点击 → `scrollIntoView({ behavior: "smooth" })` 到 WorldTopicPanel
- planned topic 卡片 → `<div>` 不可点击（不再跳到 `#worlds`）
...
| 渠道 | 方式 | 地址 |
|------|------|------|
| Vercel | `npm run build` → 自动部署 | vercel 域名 |
| 自有服务器 | `bash scripts/deploy.sh`（rsync） | https://118.145.242.99/kids/ |
...
### 已完成（2026-05-11 session）

| 项目 | 状态 |
|------|------|
| App.tsx 重构 799→23 行 | Done，33 个 TS/TSX 文件，17 个目录 |
| 校验脚本三件套 | Done，三项全绿 |
| i18n overlay 锁字段清理 | Done，55 个违规已修 |
| topic-registry 补 12 个 planned topic | Done |
| asset 路径 v01→v02 修正 | Done，20 个路径已更新 |
| 导航 UX 修复（scrollTo + scrollIntoView） | Done |
| Vercel 构建修复（跳过 PNG 配对检查） | Done |
| 服务器 + GitHub 部署 | Done |
...
| 任务 | 说明 |
|------|------|
| **新 topic 生产流程文档** | 标准化流程：topic brief → JSON → schema validate → image manifest → prompt pack → batch generate → asset import → i18n overlay → QA |
...
| **响应式 QA** | 375/768/1280 三宽度验收 |

#### P2：内容扩展（首批 6 个新 topic）

按世界优先级建议：
1. `water-cycle`（地球世界，直观、可视化强）
2. `digestion`（身体世界，与 blood-cells-3d 互补）
3. `moon-phases`（宇宙世界，观察性强）
4. `dinosaurs`（生命世界，儿童兴趣高）
5. `robots`（人造系统世界，与 llm-kids-basics 呼应）
6. `ecosystem`（生命世界，综合性主题）

每个新 topic 需要：
- 1 个 topic JSON（~15KB，遵循 schema）
- 1 个 en-US overlay JSON
- 4-6 张 WebP 图片（hero + object icons + mechanism + compare）
- 在 load-topic.ts 加 import + 映射
- 在 topic-registry.json 更新 status → render-ready
- `npm run validate` 全绿
...
- [x] 根 index.html 自动跳转到 boards/kids-world/
- [x] App.tsx 23 行，纯装配（router + locale + PageShell）
- [x] kids-world 有 7 个世界，6 个 render-ready topic，12 个 planned topic
- [x] topic 页面 11 个模块（hero → related strip），每个是独立组件文件
- [x] RepresentativeObjects 是受控组件（activeObjectId 提升到 TopicPage）
- [x] clickTask 有 3 种类型（singleChoice/findTarget/sequenceClick）
- [x] i18n overlay 有锁字段守卫（type/correctOptionId/etc 不允许出现）
- [x] 43 张 WebP 图片全部到位，0 缺图
- [x] hash 变化时 scrollTo(0,0)
- [x] 世界卡片点击 scrollIntoView 到 topic panel
- [x] planned topic 卡片不可点击
- [x] `npm run validate` 三项全绿
- [x] 新增 topic 只需：load-topic.ts 加 import + topic JSON + overlay + 图片
...
```bash
# 1. 创建 topic JSON
vim boards/kids-world/src/data/topics/{slug}.json
# 遵循 data/schema/topic.schema.json

# 2. 创建 en-US overlay
vim boards/kids-world/src/data/locales/en-US/topics/{slug}.json
# 只翻译文案字段，不包含 type/correctOptionId/targetIds 等锁字段

# 3. 更新 load-topic.ts
# 加 2 行 import + 2 行映射到 topicBaseBySlug/topicEnBySlug

# 4. 更新 topic-registry.json
# 把对应 slug 的 status 从 planned 改为 render-ready

# 5. 生成图片（用 V2.1 词包模板）
# 每个 topic 需要 4-6 张：hero + object-icons + mechanism + compare 等
# 生成后放入 boards/kids-world/public/assets/{world}/{slug}/
# 更新 image-generation-manifest.json

# 6. 更新 asset-map.ts
# topicHeroAssetBySlug 加一行映射

# 7. 校验
npm run validate  # 必须全绿

# 8. 浏览器验证
# dev server 上检查 HomePage 世界列表 + TopicPage 全部模块
```
