...
9. **`PUBLISH-01` 不可变 package**：本地已完成（只消费 `approved` 报告；revision 不可覆盖；撤回/替代保留历史）；已本地提交 `7a127b4`；未 merge 进生产。其后按路线图 §5 做 ACCEPT-01。
10. 第二台电脑、加密异地备份、浏览器会话、公网 Portal、只读 MCP、旧站替换：**后置**，不作为单人主路径门禁。
11. 兔子完整交付及第二个哺乳动物一致性验收仍依赖发布链路，不提前把 SITE-01 拉进主路径。
