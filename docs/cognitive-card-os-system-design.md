...
15. 第二台电脑、加密异地备份、浏览器会话、只读 MCP、旧 URL 映射（SITE-02）：**后置**，不作为单人主路径门禁。
16. 第二个哺乳动物一致性验收仍依赖模板族。
...
6. 当旧站知识入口已切到 Card OS 画廊，且 13 个旧主题具有明确冻结/归档决定（[ADR-005](decisions/ADR-005-knowledge-entry-cutover-without-topic-remakes.md)）后，切换主 CTA；把旧站从可达档案中删除、以及避免旧 URL 静默 404，仍要求主题重制或 SITE-02 映射；
7. 旧 `kids-world` 转为只读兼容页并逐步重定向，历史路径保留替代说明（SITE-02）。
