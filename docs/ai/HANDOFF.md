...
- Branch: main
- Base Commit: 42b3522
- Kids HEAD: add2b65 `docs(card-os): record WB-02 mapping scheme as done locally`
- Server Branch: `knowledge-pipeline-v1` @ `5c1188074846acbf8652ccb888792080697d140e`
- Working Tree: kids 仅余既有未跟踪 `outputs/`；server 仅余未跟踪 `uv.lock`
- Task Status: **Done（WB-02 已分别提交；未现网）。**

## Summary

WB-02 已提交。server `5c11880` 实现映射预览/锁定，不移动 knowledge current。kids `add2b65` 收录批准 spec、实施计划与路线图 Done。未打 release、未 reload Nginx、未改画廊。

## Completed

- server：`5c1188074846acbf8652ccb888792080697d140e` `feat(knowledge): lock projection mapping without moving current`
- kids：`add2b658bda4948cbd9e5e8c74a5ac01a0e48995` `docs(card-os): record WB-02 mapping scheme as done locally`
...
| kids | WB-02 spec / plan / 路线图 / 任务与交接 | 已提交 `add2b65` |
...
| 角色 | 路径 | 规则 |
| --- | --- | --- |
| kids 治理 | `main` @ `add2b65` | 不改 server |
...
- Kids HEAD：`add2b658bda4948cbd9e5e8c74a5ac01a0e48995`。
