...
whole-branch final review 发现 4 个 Important 与 2 个 Minor。唯一 fix wave 已把文档日期转换为严格 UTC epoch day，强制 `due > last` 且 `due - last <= 31`，并仅依据 `today - last > 31` 产生 overdue WARN；archive manifest 的两条映射改为 fail-closed 精确验证；Hook 对治理/infra 受检文件的 partial staging fail-closed，并把 `PROJECT_CONTEXT.md` 纳入纯文档豁免。re-review 发现 staged deletion 后同路径 untracked recreation 未进入 overlap 集合，本窄补丁将 `git ls-files --others --exclude-standard` 合入 worktree-only paths，并继续以 `INFRA_RE` 限定两侧集合。设计、计划与 docs map 已同步为 Approved/Implemented、Completed、Implemented/Completed。
...
- Whole-branch final-review fix wave：文档治理 fixture 扩展到 18/18，Hook fixture 扩展到 3/3；2099 due、超过 31 天、manifest 缺行/错配、staged broken/worktree clean 与 staged deletion/same-path-untracked recreation 均不能通过，恰好 31 天、PROJECT_CONTEXT-only staged 与无关 untracked 保持通过。

## Changed Files

final-review fix wave 开始时 `HEAD=1b0fe83` 且工作区 clean；content fix 为 `9e5c040`，status record 为 `ef405e8`。本 narrow re-review patch 修改 Hook、Hook fixture、Agent 文档、CURRENT_TASK 与本 HANDOFF；下表为累计 final wave 文件：
...
- Hook 的 infra 检查读取 worktree，因此治理/infra 同文件 partial staging 与 staged deletion/same-path-untracked recreation 必须拒绝；无关 untracked 保持允许，targeted Hook test 不接入统一五步入口。
...
| `npm run test:pre-commit` | PASS | 3/3：staged broken/worktree clean 与 staged deletion/same-path-untracked recreation 拒绝；PROJECT_CONTEXT-only + unrelated untracked 通过 |
